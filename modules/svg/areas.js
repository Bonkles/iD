...
export function svgAreas(projection, context) {
    ...
    function drawAreas(selection, graph, entities, filter) {
        ...
        var graphRelationEditClass = function(d) {
            if (d.type !== 'relation'){
                return ''; 
            }

            return d.members.some(function(member) {
                if (!base.entities[member.id]) {
                    return true; 
                }                
                var result = !_isEqual(_omit(graph.entities[member.id], ['tags', 'v']), _omit(base.entities[member.id], ['tags', 'v'])); 
                return result; 
            }) ? ' graphedited ': '';
        };


        var graphWayEditClass = function(d) {
            if (d.type !== 'way'){
                return ''; 
            }
        ...
        paths = paths.enter()
            ...
            .each(function(entity) {
                ...
                this.setAttribute('class', entity.type + ' area ' + layer + ' ' + graphRelationEditClass(entity) + ' ' + graphWayEditClass(entity) + ' ' + entity.id);
